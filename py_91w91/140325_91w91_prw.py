...
t  = np.linspace(0.,100,101)#,100)
u  = np.linspace(0.,100,101)#,99) 
y  = np.linspace((1.+1e-5),101,101)#,98)
y0 = np.linspace((1.+1e-5),101,101)#,98)
...
f_term0 = (1./(Y0*np.sqrt(1.0 - (1./Y0)))) *U*U*U*U\
        ...
        *(2.*(1. + 3.*a[0])*(1. + 3.*a[0])\
        +(1.-a[0])*(1.-a[0]))
#Ft_term0 = simps(f_term0, u)# axis = 1)
Ft_term0 = np.trapz(f_term0, u)# axis = 1)
#Fty_term0 = simps(Ft_term0,y)
