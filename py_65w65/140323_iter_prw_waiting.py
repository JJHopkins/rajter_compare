...
Ls = np.arange(1e-9,51e-9,10e-9)  # separation distance between 2 cyclinders
...
y = np.linspace(1.000001,100001.000001,1e4)
...
pl.figure()
pl.loglog(1e9*Ls,1e21*sum_A,label=r'$\mathcal{A}(\ell=%1.1f nm)=%3.2f, \,\,\, \mathcal{A}(\ell=%1.1f nm)=%3.2f$'%(1e9*Ls[0],1e21*sum_A[0],1e9*Ls[3],1e21*sum_A[3]))
pl.xlabel(x_ax)
pl.ylabel(y_ax_par)
...
pl.figure()
pl.semilogy(1e9*Ls,1e21*sum_A,label=r'$\mathcal{A}(\ell=%1.1f nm)=%3.2f, \,\,\, \mathcal{A}(\ell=%1.1f nm)=%3.2f$'%(1e9*Ls[0],1e21*sum_A[0],1e9*Ls[3],1e21*sum_A[3]))
pl.xlabel(x_ax)
pl.ylabel(y_ax_per)
pl.title(title('6','5','parallel'))
pl.legend(loc = 'best')
