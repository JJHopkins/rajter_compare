...
eiz_x = np.loadtxt('data/eiz_x_output_eV.txt') # len(n), LDS of cyl in perpendicular direction
eiz_z = np.loadtxt('data/eiz_z_output_eV.txt') # len(n), LDS of cyl in parallel direction
eiz_w = np.loadtxt('data/eiz_w_output_eV.txt') # len(n), LDS of water, intervening medium
...
T = np.linspace(0.,1e4,1e4)     # integration variable
U = np.linspace(0.,1e4,1e4)     # integration variable
...
np.savetxt('65w65_srw_sum_A0.txt',sum_A0)
np.savetxt('65w65_srw_sum_A2.txt',sum_A2)
np.savetxt('65w65_srw_Ls.txt',Ls)
...
pl.legend(loc = 'best')
pl.savefig(svfig('6','5','loglog, perpendicular'))
pl.show()
...
pl.savefig(svfig('6','5','perpendicular'))
pl.show()
pl.legend(loc = 'best')
pl.show()
