...
eiz_x = np.loadtxt('data/eiz_x_rr_output_eV.txt') # LDS in perpendicular direction
eiz_z = np.loadtxt('data/eiz_z_rr_output_eV.txt') # LDS in parallel direction
zs, eiz_w = np.loadtxt('data/LD_water_pars1.txt', delimiter='     ',usecols=(0,1), unpack=True)
#eiz_w = np.loadtxt('data/eiz_w_rr_L_output_eV.txt') # LDS of water, intervening medium

# Constants
c = 2.99e8              # [m/s]
coeff = 0.159#2.411e14        # [rad/s]
Temp = 297              # [K] 
#kbT = Temp * 8.6*1e-5 # 1.3807e-23 # [J]
kbT = Temp * 1.3807e-23 # [J]

# Matsubara frequencies
ns = zs/(0.159)#np.arange(0.,500.) 
#zs = ns * coeff         

# Intersurface eparation distance between 2 cyclinders
Ls = np.arange(1e-9,50e-9,1e-9)        

#Integration vars
t  = np.linspace(0.,2.**5, 1.+2.**5)
u  = np.linspace(0.,2.**5, 1.+2.**5)
y  = np.linspace((1.+1e-5),2.**5, 1.+2.**5)
y0 = np.linspace((1.+1e-5),2.**5, 1.+2.**5)
...
np.savetxt('65w65_A_parallel_ret.txt',sum_A)
np.savetxt('65w65_Lengths_parallel_ret.txt',Ls)
...
pl.legend(loc = 'best')
pl.savefig(svfig('6','5','with_RR_loglog_parallel'))
pl.show()
...
pl.savefig(svfig('6','5','with_RR_parallel'))
pl.show()
pl.legend(loc = 'best')
pl.show()
