...
pl.figure()
pl.loglog(ns,(kbT/(12.* np.pi)) * A[0,:], '-', label = r'$A(\ell=%2.1f\,nm)$'%(1e9*Ls[0]))
pl.loglog(ns,(kbT/(12.* np.pi)) * A[1,:], '-', label = r'$A(\ell=%2.1f\,nm)$'%(1e9*Ls[1]))
pl.loglog(ns,(kbT/(12.* np.pi)) * A[2,:], '-', label = r'$A(\ell=%2.1f\,nm)$'%(1e9*Ls[2]))
pl.loglog(ns,(kbT/(12.* np.pi)) * A[3,:], '-', label = r'$A(\ell=%2.1f\,nm)$'%(1e9*Ls[3]))
pl.legend(loc = 'best')
pl.title(r'290w9290 Matsubara terms')
pl.ylabel(r'$\mathcal{A}$')
pl.xlabel(r'$\ell$')
pl.savefig('290w290_A_vs_n.pdf')
pl.show()
